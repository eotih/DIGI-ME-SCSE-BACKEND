...
namespace SCSE_BACKEND.Controllers
{
    ...
    public class InterfaceController : ApiController
    {
        ...
        //  Thêm/Sửa Thông Tin Tên Ngân Hàng 
        [Route("AddOrEditFieldName")]
        [HttpPost]
        public object addOreditFieldName(FieldName1 fn1)
        {
            if (fn1.ID == 0)
            {
            ...
            {
                var obj = db.FieldName.Where(x => x.ID == fn1.ID).ToList().FirstOrDefault();
                if (obj.ID > 0)
                {
                    obj.Field = fn1.Field;
                    obj.IDField = fn1.IDField;
                    db.SaveChanges();
                    return new Response
                    {
                        Status = "Updated",
        ...
        {
            var obj = db.FieldName.Where(x => x.IDField == idfield).ToList().FirstOrDefault();
            return obj;
        }
        ...
        [Route("DeleteField")]
        [HttpDelete]
        public object deleteField(int ID)
        {
            var obj = db.FieldName.Where(x => x.ID == ID).ToList().FirstOrDefault();
            db.FieldName.Remove(obj);
            db.SaveChanges();
            return new Response
            {
                Status = "Delete",
        ...
        public object addorupdateCategory(Category1 category1)
        {
            ...
            else
            {
                ...
                if (category1.IDCat > 0)
                {
                    ...
                    obj.IDparent = category1.IDparent;
                    obj.Slug = ReplaceSpecialChars(category1.CategoryName);
                    db.SaveChanges();
                    return new Response
                    {
                        Status = "Updated",
